...
def parse_vanguard(conn, text):
    ...
    vanguard_holdings = [
        ...
        ("30267687","MGK","VANGUARD MEGA CAP GROWTH ETF",83.18,937.495,17544.33,77984.71),    # 5-for-1 split effective Apr 21 2026 (pre-split: price=415.92, qty=187.499)
        ("30267687","VCR","VANGUARD CONSUMER DISCRETIONARY ETF",398.59,68.809,1131.02,27426.66),
        ("30267687","VGT","VANGUARD INFORMATION TECH ETF",100.70,2711.304,176274.14,273021.21),  # 8-for-1 split effective Apr 21 2026 (pre-split: price=805.58, qty=338.913)
...
# ── Share split log ─────────────────────────────────────────────────────────
# Holdings snapshot date: 2026-04-18  (3 days BEFORE Vanguard's April 21 splits)
# The quantities and prices below are adjusted to post-split values so that
# yfinance live prices (which are post-split) match correctly.
#
#   Symbol | Ratio  | Pre-split price | Pre-split qty | Effective date
#   VGT    | 8-for-1|      $805.58    |     338.913   | 2026-04-21
#   MGK    | 5-for-1|      $415.92    |     187.499   | 2026-04-21
#
# Source: Vanguard announcement 2026-03-24, confirmed 247wallst.com, advisors.vanguard.com
# ─────────────────────────────────────────────────────────────────────────────
...
def _safe_dividend_yield(info: dict):
    """
    yfinance dividend yield fields are inconsistent:
      - `trailingAnnualDividendYield` : always a true decimal (0.0325 = 3.25%) — preferred
      - `dividendYield`               : sometimes decimal, sometimes already-percent — unreliable

    Strategy:
      1. Use trailingAnnualDividendYield if present (always decimal → multiply x100)
      2. Fall back to dividendYield — detect format via trailingAnnualDividendRate / price:
         compute expected decimal = rate/price, compare to raw to decide which form it is
      3. Hard cap at 15% — anything above is a data error; return None rather than garbage

    Returns a percentage float (e.g. 3.25 for 3.25%), or None.
    """
    MAX_YIELD = 15.0  # no holding in this portfolio legitimately yields above 15%

    # ── Preferred: trailingAnnualDividendYield is always a decimal in yfinance ──
    trailing = info.get("trailingAnnualDividendYield")
    if trailing is not None:
        pct = round(trailing * 100, 4)
        return pct if pct <= MAX_YIELD else None

    # ── Fallback layer 2: compute directly from rate/price (most reliable) ─────
    ...
    if rate is not None and price and price > 0:
        # rate/price gives yield as a true decimal regardless of how yfinance
        # formats dividendYield — bypasses the ambiguity entirely
        pct = round((rate / price) * 100, 4)
        return pct if pct <= MAX_YIELD else None

    # ── Fallback layer 3: dividendYield with format detection ─────────────────
    raw = info.get("dividendYield")
    if raw is None:
        return None

    # At this point we have no rate/price cross-check.
    # Heuristic: values < 0.50 must be decimal (no real asset yields 0.50%)
    #            values > 0.50 are treated as already-percent
    pct = round(raw * 100, 4) if raw < 0.50 else round(raw, 4)
    return pct if pct <= MAX_YIELD else None
...
# ─── NTFY NOTIFICATIONS ──────────────────────────────────────────────────────
# Uses ntfy.sh (open-source push notification service).
# Install the ntfy app on your phone, subscribe to your topic, done.
#
# Setup:
#   1. Install ntfy app (iOS / Android)
#   2. Subscribe to a topic name of your choosing (e.g. "dad_portfolio_123")
#   3. Set NTFY_TOPIC below to that same topic name
#
# Thresholds:
#   PRICE_ALERT_PCT  : notify if any position's price changes ±2% or more
#   YIELD_ALERT_PCT  : notify if any position's yield changes ±0.5% or more

NTFY_TOPIC       = "databaseautomation"      # ntfy.sh topic (confirmed working)
NTFY_SERVER      = "https://ntfy.sh"         # self-host or use ntfy.sh public server
PRICE_ALERT_PCT  = 1.0                       # % price change threshold
YIELD_ALERT_PCT  = 0.5                       # absolute yield change threshold (percentage points)


def _load_previous_market_data(cache_path: str) -> dict:
    """Load last run's market data from a JSON cache file."""
...
def _save_market_data_cache(market_data: dict, cache_path: str):
    """Persist current market data to JSON so next run can diff against it."""
    serialisable = {
        sym: {k: v for k, v in data.items() if k != "_info"}
    ...
    try:
        ...
        print(f"  Warning: could not save market data cache — {e}")
...
def detect_significant_changes(
    ...
    """
    Compare current vs previous market data. Returns a single result dict:

      {
        "alerts":           list[str],   # per-position alert strings
        "portfolio_value":  float,       # current total portfolio value
        "portfolio_gain":   float,       # current value - total cost basis
        "prev_value":       float,       # previous run portfolio value
        "val_chg_pct":      float,       # % change vs previous run
        "portfolio_summary": str,        # human-readable summary line
      }

    Thresholds: PRICE_ALERT_PCT (±2%) and YIELD_ALERT_PCT (±0.5pp).
    Per-position impact ($) is included in each alert string.
    """
    alerts = []

    # ── Build qty and cost lookup ─────────────────────────────────────────
    qty_map  = {}
    cost_map = {}
    bal_map  = {}
    for _, row in df_positions.iterrows():
        sym = str(row["Symbol"]).strip()
        qty_map[sym]  = float(row["Quantity"]        or 0)
        cost_map[sym] = float(row["Cost Basis"]      or 0)
        bal_map[sym]  = float(row["Current Balance"] or 0)
    ...
    for sym, cur in current.items():
        ...
        cur_price  = cur.get("live_price")
        prev_price = prev.get("live_price")
        cur_yield  = cur.get("dividend_yield")
        prev_yield = prev.get("dividend_yield")

        # Price change alert
        if cur_price and prev_price and prev_price > 1.0 and cur_price > 1.0:
            pct_chg = (cur_price - prev_price) / prev_price * 100
            if abs(pct_chg) >= PRICE_ALERT_PCT:
                qty    = qty_map.get(sym, 0)
                impact = (cur_price - prev_price) * qty
                arrow  = "▲" if pct_chg > 0 else "▼"
                sign   = "+" if impact >= 0 else ""
                alerts.append(
                    f"{arrow} {sym} price {pct_chg:+.1f}%  "
                    f"(${prev_price:,.2f} → ${cur_price:,.2f})  "
                    f"impact {sign}${impact:,.0f}"
                )

        # Yield change alert
        if cur_yield is not None and prev_yield is not None:
            yield_chg = cur_yield - prev_yield
            if abs(yield_chg) >= YIELD_ALERT_PCT:
                arrow = "▲" if yield_chg > 0 else "▼"
                alerts.append(
                    f"{arrow} {sym} yield {yield_chg:+.2f}pp  "
                    f"({prev_yield:.2f}% → {cur_yield:.2f}%)"
                )

    # ── Portfolio totals — current run ───────────────────────────────────
    # Iterate df_positions (ALL rows) not current.items() — market_data only
    # contains tradeable symbols and misses cash/bonds/non-market positions
    cur_value = 0.0
    cur_cost  = 0.0
    for sym, qty, cost, bal in zip(
        [str(r["Symbol"]).strip() for _, r in df_positions.iterrows()],
        [float(r["Quantity"]        or 0) for _, r in df_positions.iterrows()],
        [float(r["Cost Basis"]      or 0) for _, r in df_positions.iterrows()],
        [float(r["Current Balance"] or 0) for _, r in df_positions.iterrows()],
    ):
        price = current.get(sym, {}).get("live_price")
        cur_value += bal if (price is None or price == 1.0) else price * qty
        cur_cost  += cost
    cur_gain = cur_value - cur_cost

    # ── Portfolio totals — previous run ──────────────────────────────────
    prev_value = 0.0
    if previous:
        for sym, qty, bal in zip(
            [str(r["Symbol"]).strip() for _, r in df_positions.iterrows()],
            [float(r["Quantity"]        or 0) for _, r in df_positions.iterrows()],
            [float(r["Current Balance"] or 0) for _, r in df_positions.iterrows()],
        ):
            price = previous.get(sym, {}).get("live_price")
            prev_value += bal if (price is None or price == 1.0) else price * qty

    val_chg     = cur_value - prev_value
    val_chg_pct = (val_chg / prev_value * 100) if prev_value else 0.0
    val_sign    = "+" if val_chg_pct >= 0 else ""
    gain_sign   = "+" if cur_gain >= 0 else ""

    summary = (
        f"Value:   ${cur_value:>13,.0f}  ({val_sign}{val_chg_pct:.2f}% vs last run)\n"
        f"Net G/L: {gain_sign}${cur_gain:>12,.0f}  vs cost basis"
    )

    return {
        "alerts":            alerts,
        "portfolio_value":   cur_value,
        "portfolio_gain":    cur_gain,
        "prev_value":        prev_value,
        "val_chg_pct":       val_chg_pct,
...
def send_ntfy_notification(
    ...
    Accepts the result dict from detect_significant_changes — everything
    needed (portfolio value, gain/loss, alerts) is already computed there.

    ntfy priority:
      - urgent  : one or more threshold breaches
      - default : routine update (no alerts)
    """
    alerts          = result["alerts"]
    portfolio_value = result["portfolio_value"]
    portfolio_gain  = result["portfolio_gain"]
    val_chg_pct     = result["val_chg_pct"]
    ...
    if alerts:
        lines.append("")
        lines.append(f"── {len(alerts)} significant change(s) ──")
        lines.extend(alerts)

    title    = f"Portfolio — {len(alerts)} alert(s)" if alerts else "Portfolio updated"
    # always "default" minimum so every run notifies; "high" when alerts present
    priority = "high" if alerts else "default"
    tags     = ["warning"] if alerts else ["white_check_mark"]

    # ntfy on macOS Python 3.9 + LibreSSL only accepts topic + message —
    # title/priority/tags cause HTTP 400. Use ntfy header API instead:
    # X-Title, X-Priority, X-Tags are sent as headers, not in the JSON body.
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            print(f"  ntfy notification sent → topic: {topic}  (alerts: {len(alerts)})")
    except urllib.error.HTTPError as e:
        print(f"  ntfy warning: HTTP {e.code} — {e.read().decode()[:120]}")
    except Exception as e:
        print(f"  ntfy error: {e}  (notification not sent, pipeline continues)")


def compute_portfolio_totals(df_positions: "pd.DataFrame", market_data: dict) -> tuple[float, float]:
    """
    Returns (current_portfolio_value, total_net_gain_loss).
    Mirrors build_summary_sheet logic so both use the same numbers.
    """
    total_value = 0.0
    total_cost  = 0.0
    for _, row in df_positions.iterrows():
        ...
        cost  = float(row["Cost Basis"] or 0)
        bal   = float(row["Current Balance"] or 0)
        price = market_data.get(sym, {}).get("live_price")

        mkt_val = bal if (price is None or price == 1.0) else price * qty
        total_value += mkt_val
        total_cost  += cost

    return total_value, total_value - total_cost
...
def enrich_excel(path: str, market_data: dict, symbols_col_index: int = 2):
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        # Col N — Current Mkt Value: Excel formula J*E (live price x quantity)
        # For cash/MM where price == $1, fall back to Current Balance (col H)
        if price is None or price == 1.0:
            mkt_value_formula = f"=H{row_idx}"              # cash: use current balance directly
        else:
            mkt_value_formula = f"=J{row_idx}*E{row_idx}"  # live price x quantity

        # Col O — Total Gain / Loss: Excel formula N-G (current mkt value - cost basis)
        gain_formula = f"=N{row_idx}-G{row_idx}"

        values = {9: beta, 10: price, 11: dyield_pct, 12: timestamp,
                  13: ticker_type, 14: mkt_value_formula, 15: gain_formula}
        ...
        for col, val in values.items():
            ...
            if col == 10 and val is not None:
                ...
                # Col O is now an Excel formula string — Excel handles red/green
                # via the number format [RED]; set a neutral default font here
...
def build_summary_sheet(path, df_positions, market_data, april_total):
    ...
    total_cost_basis   = 0.0   # sum of G (cost basis) across all holdings
    total_initial_bal  = 0.0   # sum of H (current balance from brokerage snapshot)

    for _, row in df_positions.iterrows():
        sym  = str(row["Symbol"]).strip() if row["Symbol"] else ""
        qty  = row["Quantity"] or 0.0
        cost = row["Cost Basis"] or 0.0
        cur_bal = row["Current Balance"] or 0.0
        data = market_data.get(sym, {})
        price = data.get("live_price")
    ...
    kpis = [
        # (label, value_col_B, cost_basis_col_G, initial_bal_col_H, fmt, font)
        ("April Snapshot Value",  april_total,   None,               april_total,          CURR_FMT, DATA_FONT),
        ("Current Market Value",  total_current, total_cost_basis,   total_initial_bal,    CURR_FMT, DATA_FONT),
        ("Total Gain / Loss ($)", total_gain,    None,               None,                 '$#,##0.00;[RED]($#,##0.00)',
             GREEN_FONT if total_gain >= 0 else RED_FONT),
        ("Total Gain / Loss (%)", pct_change,    None,               None,                 PCT_FMT,
    ...
    for c, label in enumerate(["Metric", "Value", "Total Cost Basis ($)", "April Snapshot ($)"], start=1):
    ...
    for kpi in kpis:
        label, value, cost_val, init_val, fmt, font = kpi[0], kpi[1], kpi[2], kpi[3], kpi[4], kpi[5]
        ...
        # Col C — Total Cost Basis (linked from holdings col G)
        gc = ws.cell(row=row_n, column=3, value=cost_val)
        gc.font = DATA_FONT; gc.alignment = CENTER; gc.border = BORDER
        if cost_val is not None:
            gc.number_format = CURR_FMT

        # Col D — April Snapshot / Initial Balance (linked from holdings col H)
        hc = ws.cell(row=row_n, column=4, value=init_val)
        hc.font = DATA_FONT; hc.alignment = CENTER; hc.border = BORDER
        if init_val is not None:
            hc.number_format = CURR_FMT
...
if __name__ == "__main__":
    ...
    # ── Market data enrichment ────────────────────────────────────────────
    CACHE_PATH = "market_data_cache.json"

    print("\nFetching live market data (Beta, Price, Yield)…")
    prev_market_data = _load_previous_market_data(CACHE_PATH)
    ...
    # ── Detect changes, compute portfolio totals, notify ────────────────
    print("\nChecking for significant price/yield changes…")
    result = detect_significant_changes(market_data, prev_market_data, df_final)

    print(f"  {result['portfolio_summary']}")
    if result["alerts"]:
        print(f"  {len(result['alerts'])} alert(s) detected:")
        for a in result["alerts"]:
            print(f"    {a}")
    else:
        print("  No significant changes vs last run.")

    send_ntfy_notification(result)

    # ── Save cache for next run ───────────────────────────────────────────
    _save_market_data_cache(market_data, CACHE_PATH)
    print(f"  Market data cached → {CACHE_PATH}")
